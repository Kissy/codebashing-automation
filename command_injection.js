    ...
    if (!activeMenu) {
        ...
        switch (activeMenu.textContent) {
            case "1":
                ...
                break;
            case "2":
                clickIfAvailable('div#-run-analysis-');
                clickIfAvailable('button.next-button');
                break;
            case "3":
                ...
                break;
            case "4":
                var react = findReact(document.querySelector('div.-fake-browser-search-bar- input'), 3);
                if (react) {
                    react.setState({value: "https://tradeadmin.codebashing.com/console/execute?username=alice;id"});
                    //react.onSubmit()
                }
                break;
            case "5":
            ...
            case "9":
                ...
                if (react) {
                    react.setState({userName: "alice@bank.com", password: "' or 1=1)#"});
                    react.onSubmit()
                }
                break;
            case "10":
